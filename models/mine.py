...
class Mine(nn.Module):
    ...
    def forward(self, x, z, j, z_marg=None):
        if z_marg is None:
            z_marg = z[torch.randperm(x.shape[0])]
        
        if torch.all(torch.eq(j, j[0])):
          t = self.T(x, z).mean()
          ...
          if self.loss in ['mine']:
          ...
          elif self.loss in ['JSD']:
              second_term = t-(torch.mean(-
                                          F.softplus(- self.T(x, z), beta=1, 
                                                     threshold=20)+ math.log(2))-
                               torch.mean(F.softplus(-t_marg, beta=1,
                                                   threshold=20)-math.log(2)+
                                          t_marg))
            ...
            t = (self.T(x, z).reshape(1,self.T(x, z).shape[0])*
                 j.reshape(1,self.T(x, z).shape[0])).sum()
            t_marg = self.T(x, z_marg)
            ...
            if self.loss in ['fdiv']:
                ...
                second_term = t-(((-F.softplus(- self.T(x, z), beta=1, 
                                                      threshold=20)+ math.log(2)).reshape(1,self.T(x, z).shape[0])*                                                                                       (j.reshape(1,self.T(x,z).shape[0]))).sum() -
                                ((F.softplus(-t_marg, beta=1,
                                                      threshold=20)-math.log(2)+
                                            t_marg).reshape(1,self.T(x, z).shape[0])*(j.reshape(1,self.T(x,z).shape[0]))).sum())
            elif self.loss in ['mine_biased']:
                J_log=torch.log(j.reshape(self.T(x, z).shape[0],1))
                second_term = torch.logsumexp(J_log+
                                              t_marg.reshape(t_marg.shape[0],1),
                                              0)

        return -t + second_term

    def mi(self, x, z, j, z_marg=None):
...
class Mine_real(nn.Module):
    def __init__(self, T_real, loss='mine', alpha=0.01, method=None):
        super().__init__()
        self.running_mean = 0
        self.loss = loss
        self.alpha = alpha
        ...
        if method == 'concat':
            if isinstance(T_real, nn.Sequential):
                self.T_real = CustomSequential(ConcatLayer(), *T_real)
            else:
                self.T_real = CustomSequential(ConcatLayer(), T_real)
        else:
            self.T_real = T_real
    ...
    def forward(self, x, z, j, z_marg=None):
        ...
        if torch.all(torch.eq(j, j[0])):
          t = self.T_real(x, z).mean()
          t_marg = self.T_real(x, z_marg)
          ...
          if self.loss in ['mine']:
              ...
              second_term = t+(torch.mean(-
                                          F.softplus(- self.T_real(x, z), beta=1, 
                                                     threshold=20)+ math.log(2))-
                               torch.mean(F.softplus(-t_marg, beta=1,
                                                   threshold=20)-math.log(2)+
                                          t_marg))
            ...
            #j=torch.tensor(j,dtype=torch.float32)
            t = (self.T_real(x, z).reshape(1,self.T_real(x, z).shape[0])*
                 j.reshape(1,self.T_real(x, z).shape[0])).sum()
            t_marg = self.T_real(x, z_marg)
            ...
            if self.loss in ['fdiv']:
                ...
                J_log=torch.log(j.reshape(self.T_real(x, z).shape[0],1))
                second_term = torch.logsumexp(J_log+
                                              t_marg.reshape(t_marg.shape[0],1),
                                              0)
...
class T_real(nn.Module):
    def __init__(self, x_dim, z_dim):
        super(T_real, self).__init__()
        self.layers = CustomSequential(ConcatLayer(), nn.Linear(x_dim + z_dim, 400),
                                       nn.ReLU(),
                                       nn.Linear(400, 400),
                                       nn.ReLU(),
